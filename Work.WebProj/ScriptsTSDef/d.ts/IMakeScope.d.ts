...
interface INGScope extends ng.IScope {
    ...
    Master_Open_New(): void;
    //sub detail
    Detail_Init($index: number): void;
    Sub_Detail_Delete($index: number): void;
    SelectAllSubCheckDel($index: number, $event: any): void;
    Sub_Submit(): void;
    Sub_Edit_Close(): void;
    Sub_Open_Modify($parentIndex: number, $index: number): void;
    Sub_Open_New($index: number): void;//新增sub要知道是哪個Master下新增$index
}
